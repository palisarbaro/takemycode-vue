    ...
    <div ref="tags" class="multiselect__tags">
      ...
      >
        <div class="multiselect__tags-wrap" v-show="visibleValues.length > 0">
          <span :ref="'dragRef0'" :class="['multiselect__tag','dropzone']"
                @drop="onDrop($event, 0)"
                @dragenter.prevent="onDragEnter(0)"
                @dragleave.prevent="onDragLeave(0)"
                @dragover.prevent
              >
                •
              </span>
          <template v-for="(option, index) of visibleValues" >
            <slot name="tag" :option="option" :search="search" :remove="removeElement">
              <span class="multiselect__tag" :key="index" @dragstart="onDragStart($event, index)" draggable="true">
              ...
              <span :class="['multiselect__tag','dropzone']" :key="index" :ref="'dragRef'+(index+1)"
                @drop="onDrop($event, index+1)"
                @dragenter.prevent="onDragEnter(index+1)"
                @dragleave.prevent="onDragLeave(index+1)"
                @dragover.prevent
              >
                •
              </span>
...
export default {
  ...
  methods: {
    ...
    arrayMove (arr, fromIndex, toIndex) {
      ...
      const element = res[fromIndex]
      res.splice(fromIndex, 1)
      res.splice(toIndex, 0, element)
      return res
    },
    onDrop (event, to) {
      const from = Number.parseInt(event.dataTransfer.getData('index'))
      for (const ref of this.refs) {
        ref.style.opacity = '0%'
      }
      const newValue = this.arrayMove(this.modelValue, from, to)
      this.$emit('update:modelValue', newValue)
    },
    onDragEnter (index) {
      let elem = this.$refs['dragRef' + index]
      elem = elem[0] || elem
      this.refs.add(elem)
